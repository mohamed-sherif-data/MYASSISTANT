...
class QuizWindow:
    ...
    def show(self) -> None:
        ...
        setup.geometry(f"420x480+{(sw-420)//2}+{(sh-480)//2}")
        setup.minsize(380, 400)
        ...
        for label, value in QUIZ_MODES:
            tk.Radiobutton(f, text=label, variable=mode_var, value=value,
                           ...
                           activebackground=C["bg"], font=("Arial", 10)).pack(anchor="w", pady=2)
        ...
        tk.Label(f, text="المادة:", bg=C["bg"], fg=C["text2"],
                 font=("Arial", 9)).pack(anchor="w")
        subjects = ["الكل"] + sorted({w.get("subject","") for w in self.app.db.load_words() if w.get("subject")})
        ...
        ttk.Combobox(f, textvariable=subj_var, values=subjects,
                     state="readonly", font=("Arial", 10)).pack(fill="x", pady=(4, 8))

        # عدد الأسئلة
        tk.Label(f, text="عدد الأسئلة:", bg=C["bg"], fg=C["text2"],
                 font=("Arial", 9)).pack(anchor="w")
        count_var = tk.IntVar(value=10)
        ttk.Spinbox(f, from_=5, to=50, textvariable=count_var,
                    font=("Arial", 10), width=6).pack(anchor="w", pady=(4, 8))

        # فلتر المستوى
        tk.Label(f, text="مستوى الكلمات:", bg=C["bg"], fg=C["text2"],
                 font=("Arial", 9)).pack(anchor="w")
        ...
        def _start():
            ...
            if subj_var.get() != "الكل":
                all_words = [w for w in all_words if w.get("subject","") == subj_var.get()]
            if lvl_var.get() != "الكل":
                all_words = [w for w in all_words if w.get("level","") == lvl_var.get()]

            if len(all_words) < 4:
                status_lbl.config(text=f"⚠ تحتاج 4 كلمات على الأقل! (لديك {len(all_words)})")
                return
            random.shuffle(all_words)
            n = min(count_var.get(), len(all_words))
            self.words = all_words[:n]
        ...
        tk.Button(f, text="🚀 بدء الاختبار", command=_start,
                  ...
                  relief="flat", padx=24, pady=10, cursor="hand2").pack(pady=(4, 0))
    ...
    def _build_ui(self) -> None:
        ...
        top.pack(fill="x")
        self.lbl_prog = tk.Label(top, text="", bg=C["surface"], fg=C["text2"], font=("Consolas", 10))
        self.lbl_prog.pack(side="left")
        self.lbl_score = tk.Label(top, text="✅ 0  ❌ 0", bg=C["surface"], fg=C["text2"], font=("Consolas", 10))
        self.lbl_score.pack(side="right")
        self.pbar_var = tk.DoubleVar()
        ttk.Progressbar(self.win, variable=self.pbar_var, maximum=len(self.words)).pack(fill="x")

        # منطقة السؤال
        q_area = tk.Frame(self.win, bg=C["bg"])
        q_area.pack(fill="both", expand=True, padx=20, pady=14)

        self.q_card = tk.Frame(q_area, bg=C["card_bg"], highlightbackground=C["border"], highlightthickness=1)
        self.q_card.pack(fill="x", pady=(0, 14))
        self.q_inner = tk.Frame(self.q_card, bg=C["card_bg"], padx=20, pady=18)
        self.q_inner.pack(fill="x")

        self.lbl_question = tk.Label(self.q_inner, text="", bg=C["card_bg"],
                                     fg=C["text"], font=("Georgia", 32, "bold"),
                                     wraplength=560, justify="center")
        self.lbl_question.pack(pady=(4, 6))
        self.lbl_ipa = tk.Label(self.q_inner, text="", bg=C["card_bg"],
                                fg=C["text3"], font=("Arial", 11, "italic"))
        self.lbl_ipa.pack()
        self.btn_speak = tk.Button(self.q_inner, text="🔊 نطق", command=self._speak_current,
                                    bg=C["surface2"], fg=C["blue"], font=("Arial", 9),
                                    relief="flat", padx=10, pady=3, cursor="hand2")
        self.btn_speak.pack(pady=(6, 0))

        self.ans_frame = tk.Frame(q_area, bg=C["bg"])
        self.ans_frame.pack(fill="both", expand=True)

    # ─ـ تحميل السؤال حسب النمط ──────────────────────────────────────────
    def _load_question(self) -> None:
        ...
        if self.mode == "listen":
            ...
            if HAS_TTS:
                threading.Timer(0.5, lambda: self.app.tts.speak_word(w.get("word",""))).start()
            ...
            self.lbl_question.config(text=f"رتّب الجملة:\n{w.get('translation','')}",
                                     fg=C["text"], font=("Arial", 15, "bold"))
            ...
            self.lbl_question.config(text=w.get("translation",""), fg=C["text"],
                                     font=("Georgia", 32, "bold"))
            self.lbl_ipa.config(text="")
            self.btn_speak.config(state="disabled")

        # تنظيف منطقة الإجابة وبناء النمط
    ...
    # ─ـ MC / Listen (اختيارات) ──────────────────────────────────────────
    def _build_mc(self, correct: dict) -> None:
        all_words = self.app.db.load_words()
        distractors = [w for w in all_words if w.get("word","") != correct.get("word","")]
        ...
        for text, is_correct in options:
            btn = tk.Button(self.ans_frame, text=f"  {text}",
                            ...
                            command=lambda t=text, c=is_correct: self._mc_answer(c))
    ...
    def _mc_answer(self, is_correct: bool) -> None:
        ...
        for btn, correct in self._mc_buttons:
            ...
            if correct:
                ...
                # FIX: كان elif not correct وهو دائماً True — أصبح else
    ...
    def _get_sound_alikes(self, correct: dict) -> list[str]:
        ...
        # fallback: عشوائية من قاعدة البيانات
        others = [w.get("word","") for w in self.app.db.load_words() if w.get("word","") != word]
        random.shuffle(others)
        return others[:4]

    # ─ـ كتابة ────────────────────────────────────────────────────────────
    def _build_write(self, word: dict) -> None:
        f = tk.Frame(self.ans_frame, bg=C["bg"]); f.pack(fill="x", pady=10)
        tk.Label(f, text="اكتب الترجمة بالعربي:", bg=C["bg"], fg=C["text2"], font=("Arial", 10)).pack(anchor="w")
        self.write_var = tk.StringVar()
        self.write_entry = tk.Entry(f, textvariable=self.write_var, bg=C["surface2"], fg=C["text"],
                                    font=("Arial", 16), relief="flat", bd=10, justify="right")
    ...
    def _build_dictation(self, word: dict) -> None:
        f = tk.Frame(self.ans_frame, bg=C["bg"]); f.pack(fill="x", pady=10)
        tk.Label(f, text="اكتب الكلمة التي سمعتها:", bg=C["bg"], fg=C["text2"], font=("Arial", 10)).pack()
        self.dict_var = tk.StringVar()
        self.dict_entry = tk.Entry(f, textvariable=self.dict_var, bg=C["surface2"], fg=C["text"],
                                   font=("Consolas", 18), relief="flat", bd=10, justify="center")
    ...
    def _check_dictation(self) -> None:
        ...
        is_correct = re.sub(r"[^a-zA-Z]","",user_ans).lower() == re.sub(r"[^a-zA-Z]","",correct_w).lower()
    ...
    def _build_word_order(self, word: dict) -> None:
        ...
        outer = tk.Frame(self.ans_frame, bg=C["bg"]); outer.pack(fill="both", expand=True)
        ...
        words_f = tk.Frame(outer, bg=C["bg"]); words_f.pack(fill="x")
        for i, tok in enumerate(shuffled):
            btn = tk.Button(words_f, text=tok, bg=C["surface"], fg=C["text"],
                            ...
                            cursor="hand2", command=lambda t=tok, idx=i: self._wo_pick(t, idx))
            btn.pack(side="left", padx=4, pady=4)
            self._wo_btns[i] = btn

        ctrl = tk.Frame(outer, bg=C["bg"]); ctrl.pack(fill="x", pady=(8,0))
    ...
    def _check_word_order(self) -> None:
        if self.answered or not self._wo_selected: return
        self.answered = True
        self.total_answered += 1
    ...
    def _build_listen(self, word: dict) -> None:
        """FIX: كانت هذه الدالة مرجعية في الـ builders dict لكنها لم تكن معرَّفة."""
        # نفس منطق MC لكن الخيارات كلمات متشابهة صوتياً
        self._build_mc(word)
    ...
    def _build_fill_blank(self, word: dict) -> None:
        ...
        tk.Label(outer, text=blanked, bg=C["surface2"], fg=C["text"],
                 font=("Georgia",15), wraplength=560, justify="center",
                 padx=20, pady=16).pack(fill="x", pady=(0,16))

        all_words = self.app.db.load_words()
        distractors = [w for w in all_words if w.get("word","") != correct_w]
        random.shuffle(distractors)
    ...
    def _show_next_btn(self) -> None:
        f = tk.Frame(self.ans_frame, bg=C["bg"]); f.pack(fill="x", pady=(10,0))
    ...
    def _show_summary(self) -> None:
        ...
        for w in self.win.winfo_children():
            w.destroy()
        f = tk.Frame(self.win, bg=C["bg"]); f.pack(fill="both", expand=True, padx=30, pady=20)
        ...
        for label, value, color in [
            ...
            row = tk.Frame(g, bg=C["surface"]); row.pack(fill="x", pady=4)
        ...
        tk.Button(f, text="🔄 اختبار جديد",
                  command=lambda: [self.win.destroy(), self.show()],
                  bg=C["primary"], fg="white", font=("Arial",11,"bold"),
                  relief="flat", padx=20, pady=8, cursor="hand2").pack(side="left", padx=6, pady=10)
